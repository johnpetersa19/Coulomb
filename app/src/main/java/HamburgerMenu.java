...
public class HamburgerMenu extends Popover {
    ...
    HamburgerMenu(){
        ...
        box.append(new TitledScale("Velocidade de simulação" , Main.simulation_speed_controller , 1 , 100, 1 , (Double value)->{
            Main.simulation_speed_controller = (int)limit(value, 1, 100);
            return null;
        }));

        box.append(new TitledScale("Velocidade atual" , CurrentDrawer.speed_controller , 1 , 20 , 0.2 , (Double value)->{
        ...
        Runnable saveAs = ()->{
            ...
            FileChooserNative fileChooserDialog = new FileChooserNative("Salvar circuito como" , Main.app.window , FileChooserAction.SAVE , null , null);
            fileChooserDialog.setModal(true);
            fileChooserDialog.show();
            fileChooserDialog.onResponse((int response)->{
                System.out.println("Salvando....");
                if(response == ResponseType.ACCEPT){
                    System.out.println("Realmente economizando...");
        ...
        Runnable open = ()->{
            ...
            FileChooserNative fileChooserDialog = new FileChooserNative("Escolha um circuito" , Main.app.window , FileChooserAction.OPEN , null , null);
            fileChooserDialog.setModal(true);
            fileChooserDialog.show();
            fileChooserDialog.onResponse((int response)->{
        ...
        box.append(new MenuButton("Exemplos" , Align.START , ()->{
            stack.setVisibleChild(examplesBox);
        }));
        box.append(new MenuButton("Salvar" , Align.START , ()->{
        ...
        box.append(new MenuButton("Salvar como" , Align.START , saveAs , this));
        box.append(new MenuButton("Abrir" , Align.START , open , this));

        box.append(new MenuButton("Claro" , Align.START , ()->{
        ...
        box.append(new MenuButton("Atalhos de teclado", Align.START, ()->{
            new MyShortuctsWindow(Main.app.window).show();
        }, this));

        //box.append(new MenuButton("Preferences" , Align.START , ()->{}));
        box.append(new MenuButton("Sobre" , Align.START , ()->{
